...
# --- 🆕 淘汰母猪折价率：深度去化的领先指标（视频补充） ---
# 淘汰母猪价格 / 活猪价格 = 折价比率
# 历史规律：折价率降至 50%（淘汰价=活猪价一半）是深度去化的确认信号
# 数据来源：涌益咨询、我的钢铁网（月度监测）
SOW_CULL_RATIO = [
    ("2025-06", 72, "正常淘汰，无去化压力"),
    ("2025-09", 70, "猪价下行，淘汰略增"),
    ("2025-12", 68, "亏损季，淘汰开始加速"),
    ("2026-03", 65, "Q1深度亏损，部分母猪淘汰"),
    ("2026-06", 60, "去化进行中，接近深度去化阈值边缘"),
    # 阈值参考线
    ("深度去化确认线", 50, "淘汰价=活猪价一半 → 史上每轮底部均触及此线"),
    ("正常淘汰线", 75, "正常更新替代，无去化信号"),
]

# --- 🆕 36家猪肉概念股筛选（视频补充） ---
# 来源：同花顺/东方财富"猪肉概念"板块，剔除弱相关公司后仅17家强正相关
CONCEPT_STOCK_FILTER = {
    "total_listed": 36,
    "strong_correlation": 17,
    "excluded": 19,
    "excluded_examples": [
        ("双汇发展", "屠宰加工为主，猪价涨=成本涨（负相关）"),
        ("海大集团", "饲料为主，养殖占比小"),
        ("立华股份", "黄羽鸡为主，非养猪"),
        ("播恩集团", "饲料添加剂，非养殖"),
        ("金河生物", "兽药+动物保健"),
    ],
    "note": "仅17家与生猪强正相关——投资者应先做标的筛选再行分析"
}

# --- 🆕 成本口径差异（视频补充） ---
# 上市公司成本披露口径五花八门，投资者无法从报表精准反推
COST_CATEGORIES = [
    ("完全成本", "含折旧摊销+三费+总部费用", "最全面，牧原/巨星以此口径披露", "⭐⭐⭐⭐⭐"),
    ("营业总成本", "含折旧但不一定含总部费用分摊", "唐人神以此口径披露", "⭐⭐⭐⭐"),
    ("运营场线成本", "仅养殖环节，不包含总部费用", "部分公司混淆使用", "⭐⭐⭐"),
    ("现金成本", "完全成本 - 折旧摊销（非付现）", "评估能否扛过底部的关键指标", "⭐⭐⭐⭐⭐"),
]
COST_NOTE = "⚠ 牧原完全成本11.6元/kg ≠ 现金成本。折旧摊销约4.2-4.3元/kg，现金成本估算≈7.3-7.4元/kg。当猪价>现金成本时企业不亏现金→去化放缓。"

# --- 🆕 集团场 vs 散养户结构变迁（视频补充） ---
STRUCTURE_SHIFT = [
    # (年份, 散养户占比%, 规模场占比%, 集团场占比%, 备注)
    (2010, 65, 30, 5, "散养户主导时代"),
    (2015, 55, 35, 10, "规模化起步，环保清退散户"),
    (2018, 49, 38, 13, "非瘟前，规模化加速"),
    (2020, 35, 43, 22, "非瘟后集团场大扩张"),
    (2023, 25, 43, 32, "集团场已成主力"),
    (2025, 18, 42, 40, "集团场主导市场格局"),
]

# --- 🆕 社交媒体干扰猪周期的新变量（视频补充） ---
SOCIAL_MEDIA_FACTOR = {
    "platforms": ["抖音", "快手", "微信公众号", "微信群/QQ群"],
    "mechanism": "大量博主鼓吹「抄底」「二次育肥」→ 散养户受情绪驱动压栏+投机 → 人为延后产能出清 → 加剧周期波动烈度",
    "impact": "移动互联网时代猪周期的新变量，传统分析框架未覆盖。短期猪价反弹（如7月初）更多是抖音情绪驱动+二育投机，而非基本面反转。",
    "mitigation": "关注出栏均重变化（压栏指标）+ 标肥价差（判断二育热度）"
}
...
# --- 🆕 图表：淘汰母猪折价率 — 深度去化的领先指标 ---
def chart_sow_cull_ratio():
    """淘汰母猪折价率趋势 + 关键阈值线（视频补充）。"""
    fig = go.Figure()

    data_points = [r for r in SOW_CULL_RATIO if isinstance(r[0], str) and not r[0].startswith("深度") and not r[0].startswith("正常")]
    months = [r[0] for r in data_points]
    ratios = [r[1] for r in data_points]
    notes = [r[2] for r in data_points]

    fig.add_trace(go.Scatter(
        x=months, y=ratios, mode="lines+markers",
        marker=dict(size=8, color="#c0392b"), line=dict(width=2.5, color="#c0392b"),
        name="淘汰母猪折价率", hovertemplate="%{x}<br>折价率=%{y}%<extra></extra>",
    ))

    # 关键阈值线
    fig.add_hline(y=75, line_dash="dash", line_color="#27ae60", line_width=1.5,
                  annotation_text="正常淘汰线 75%", annotation_position="right")
    fig.add_hline(y=50, line_dash="dash", line_color="#e74c3c", line_width=2,
                  annotation_text="深度去化确认线 50%", annotation_position="right")

    fig.update_layout(
        title="淘汰母猪折价率 — 深度去化的领先指标（🆕 视频补充）",
        height=300, margin=dict(l=40, r=40, t=50, b=35),
        xaxis=dict(title=""), yaxis=dict(title="折价率 %", range=[40, 85], ticksuffix="%"),
        plot_bgcolor="white",
    )
    return fig.to_html(full_html=False, include_plotlyjs=False, div_id="chart_sow_cull")


# --- 🆕 图表：集团场 vs 散养户结构变迁 ---
def chart_structure_shift():
    """养殖主体结构变迁——集团场主导是周期变形的根本原因（视频补充）。"""
    fig = go.Figure()

    years = [s[0] for s in STRUCTURE_SHIFT]
    small_pct = [s[1] for s in STRUCTURE_SHIFT]
    medium_pct = [s[2] for s in STRUCTURE_SHIFT]
    large_pct = [s[3] for s in STRUCTURE_SHIFT]

    fig.add_trace(go.Bar(
        x=years, y=small_pct, name="散养户",
        marker_color="#e74c3c", text=[f"{v}%" for v in small_pct], textposition="inside",
    ))
    fig.add_trace(go.Bar(
        x=years, y=medium_pct, name="规模场",
        marker_color="#f39c12", text=[f"{v}%" for v in medium_pct], textposition="inside",
    ))
    fig.add_trace(go.Bar(
        x=years, y=large_pct, name="集团场（TOP20）",
        marker_color="#2ecc71", text=[f"{v}%" for v in large_pct], textposition="inside",
    ))

    fig.update_layout(
        title="养殖主体结构变迁 — 集团场从5%→40%（🆕 视频补充）",
        height=320, barmode="stack",
        margin=dict(l=40, r=40, t=50, b=35),
        yaxis=dict(title="占比 %", ticksuffix="%", range=[0, 105]),
        legend=dict(orientation="h", y=1.12),
        plot_bgcolor="white",
    )
    return fig.to_html(full_html=False, include_plotlyjs=False, div_id="chart_structure")
...
def build_html_report():
    charts = {
        ...
        "sow_cull": chart_sow_cull_ratio(),
        "structure": chart_structure_shift(),
    }
...
<div class="container">
  ...
  <div class="section">
    ...
    <h3>🆕 集团场主导 — 本轮周期变形的根本原因（视频研究补充）</h3>
    {charts['structure']}
    <p style="font-size:12px;color:#999;margin-top:4px;">结构变迁数据来源：中国畜牧业协会、农业农村部、上市公司公告综合估计。集团场占比基于 TOP20 出栏量/全国总出栏。</p>
    <ul>
      <li><b>核心判断</b>：集团场从 2010 年的 5% 扩张至 2025 年的约 40%，市场格局已从「散养户主导」变为「集团场主导」——<b>这是本轮猪周期打破一切历史规律的根本原因</b></li>
      <li><b>三大去化因素同时弱化</b>：疫病——集团场生物安全防控远超散户（非瘟散点爆发但不致命）；环保——大场合规提前布局，小场已被清退殆尽；亏损——集团场资金雄厚+融资渠道多+固定成本高退出难，<b>硬扛不退出</b></li>
      <li><b>固定成本陷阱</b>：集团场猪舍/设备折旧占比高（牧原折旧 4.2 元/kg），固定资产一旦建成无法变现——停产=巨亏，不停产=少亏→理性选择是硬扛，直到现金流断裂</li>
      <li><b>反转信号</b>：只有看到集团场大面积爆雷/重整，才是真正周期反转的信号。目前仅个别边缘企业（如天邦/天域）岌岌可危，龙头（牧原/温氏）现金储备充裕</li>
    </ul>
  </div>
  ...
  <div class="section">
    ...
    <h3>🆕 淘汰母猪折价率 — 深度去化的领先指标（视频研究补充）</h3>
    {charts['sow_cull']}
    <p style="font-size:12px;color:#999;margin-top:4px;">折价率 = 淘汰母猪价格 ÷ 活猪价格 × 100%。数据来源：涌益咨询/我的钢铁网月度监测。折价率越低=淘汰越剧烈。</p>
    <ul>
      <li><b>核心逻辑</b>：淘汰母猪折价率降至 <b>50%</b>（淘汰价=活猪价一半）是历史上每轮周期底部都触及的深度去化确认线。当前折价率约 <b>60%</b>——去化正在进行，但尚未进入深度淘汰阶段</li>
      <li>淘汰母猪价格来自生猪屠宰场收购价，不依赖于上市公司披露，<b>数据真实性高</b></li>
      <li>当折价率从 60% 降至 50% 的过程，通常伴随能繁母猪存栏加速下降——与 2026Q2 的 -3.2% 环比降幅吻合</li>
      <li><b>当前判断</b>：去化在加速但尚未出清——折价率仍有 10 个百分点的下行空间，对应约 <b>100-150 万头</b>能繁母猪可被淘汰</li>
    </ul>
...
<tr><td><b>牧原股份</b></td><td>11.3</td><td><b>11.6</b></td><td>&lt;11.5</td><td>Q2 约 11.6，优秀场线 &lt;11</td></tr><tr><td>温氏股份</td><td>12.2</td><td><b>~12.0</b></td><td>~11.8</td><td>Q2 约 12，Q1 账面 12.4</td></tr><tr><td>新希望</td><td>12.7</td><td>—</td><td>—</td><td>2025 年末数据，2026 未更新</td></tr><tr><td>正邦科技</td><td>13.3</td><td>—</td><td>—</td><td>2025 年末数据</td></tr><tr><td>天邦食品</td><td>13.4</td><td>—</td><td>—</td><td>2025 年末数据</td></tr><tr><td>上市猪企平均</td><td>12.9</td><td>—</td><td>—</td><td>15 家均值（2025 末）</td></tr><tr><td>中小散户</td><td>17.0</td><td>—</td><td>—</td><td>成本劣势 4~6 元/kg</td></tr></table>
    ...
    <div class="callout" style="background:#fff3cd; border-left:4px solid #f39c12; padding:12px 16px; margin:16px 0; border-radius:4px;">
      <b>⚠ 成本口径警示（🆕 视频研究补充）</b><br>
      <p style="margin:6px 0 0 0;font-size:13px;">
      上市公司成本披露口径<b>五花八门</b>，不可直接对比：
      <b>完全成本</b>（牧原/巨星）包含折旧+三费+总部费用；
      <b>营业总成本</b>（唐人神）不含总部费用分摊；
      <b>运营场线成本</b>仅养殖环节；
      <b>现金成本</b> = 完全成本 - 折旧摊销（非付现）。
      牧原完全成本 11.6 元/kg，但折旧摊销约 4.2 元/kg → <b>现金成本仅约 7.3-7.4 元/kg</b>。
      当猪价 &gt; 现金成本时（当前猪价 10.5 &gt; 7.4），牧原<b>不亏现金</b>→ 可以硬扛不出清。
      这一洞察解释了"为什么底部如此漫长"——<b>多数上市猪企现金成本远低于完全成本，亏损但不致命。</b>
      </p>
    </div>
  ...
  <div class="section">
    ...
    <p class="source">综合本报告 §4-§5 数据，将期货、猪粮比、能繁去化、PSY、仔猪、淘汰母猪折价率六条独立信号线交叉验证。🆕 新增淘汰母猪折价率来自视频研究补充。</p>
    ...
    <table>
      ...
      <tr><td>🆕 淘汰母猪折价率</td><td>~60%（vs 深度去化阈值 50%）</td><td style="color:#f39c12">◀ 中性偏多</td><td>去化在加速但尚未出清，仍有下行空间</td><td>高（屠宰场实价，非披露数据）</td></tr>
    ...
    <div class="box">
      <h3>六条独立信号线中，五条指向同一个方向：猪周期上行</h3>
      <p>唯一反向信号（PSY 提升）只是<b>缓冲</b>去化的效果，并不否定去化方向。淘汰母猪折价率（新增信号）提示去化在加速但未出清——与能繁母猪 Q2 加速下降形成交叉验证。核心逻辑链：</p>
      <p><b>猪粮比 4.41 → 深度亏损不可持续 → 能繁加速去化（Q2 -3.2%）+ 淘汰母猪折价率下行（60%→目标50%）→ 6-10 个月后供给收缩 → 猪价回升至成本线上方 → 牧原率先盈利</b></p>
    ...
    <ol>
      ...
      <li><b>🆕 社交媒体/二次育肥干扰（视频补充）</b>：抖音、快手上大量博主鼓吹「抄底」「二次育肥」→ 散养户受情绪驱动压栏+投机 → 人为延后产能出清 → 拉长周期底部。7 月初猪价短暂反弹就是典型案例（抖音情绪驱动 + 二育投机，非基本面反转）。概率：中高（35%）。应对：关注出栏均重变化（压栏指标）+ 标肥价差（判断二育热度），而非盲信短期猪价反弹</li>
      <li><b>大规模疫情</b>：非瘟变异株或新疫情 → 牧原高密度养殖模式生物安全风险。概率：低（10%），但影响极大。应对：分散场区、全封闭管理</li>
    </ol>

    <h3>🆕 研究提示：36家"猪肉概念股"中仅17家与生猪强相关（视频补充）</h3>
    <p style="font-size:13px;color:#666;">
    在同花顺/东方财富的36家「猪肉概念」板块中，仅<b>17家</b>与生猪养殖强正相关。其余19家（如双汇发展——屠宰加工，猪价涨=成本涨；海大集团——饲料为主；立华股份——黄羽鸡）与猪价的关联逻辑完全不同。进行养猪行业投资时，必须先做标的筛选，排除弱相关公司后再行分析和估值对比。本项目聚焦牧原股份（纯猪企龙头），不受此问题影响，但投资者在横向对比同行时需注意此筛选步骤。
    </p>
  </div>
