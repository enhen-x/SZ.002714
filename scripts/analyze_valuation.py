...
def ch4c_price_ttm_pe_trend():
    ...
    # 周期均值PE参考线（价格÷2.04）
    cycle_pe_ref = CURRENT_PRICE / AVG8_EPS
    fig.add_shape(type="line", x0=x_l, x1=x_r,
                  y0=cycle_pe_ref, y1=cycle_pe_ref, yref="y2",
                  line=dict(color=C["gold"], width=1.2, dash="dot"), opacity=0.6)
    fig.add_annotation(x=0.5, y=cycle_pe_ref, yref="y2",
                      text=f"当前周期均值PE {cycle_pe_ref:.1f}×", showarrow=False,
                      font=dict(size=9, color=C["gold"]), xanchor="left",
    ...
    # 动态计算各阶段平均PE
    def avg_pe_in_range(start_q, end_q):
        vals = []
        for i in range(len(qtr)):
            lbl = q_labels[i]
...
def ch11_historical_validation():
    ...
    # ── 5b. 近4季真实EPS × 12×（反映市场实际定价行为：市场通常对近期利润而非周期均值定价）──
    eps_q = qtr["EPSJB"].tolist()
    trailing_4q_eps = []
    for i in range(n):
        window_eps = [v for v in eps_q[max(0, i-3):i+1] if pd.notna(v)]
        if len(window_eps) >= 2:
            trailing_4q_eps.append(sum(window_eps) * (4 / len(window_eps)))  # 年化
        else:
            trailing_4q_eps.append(None)
    trailing_pe_12 = []
    for i in range(n):
        if trailing_4q_eps[i] and trailing_4q_eps[i] > 0:
            trailing_pe_12.append(trailing_4q_eps[i] * 12)
        else:
...
def main():
    ...
    chart_funcs = [
        ...
        ("price_cycle_pe_ch4c", ch4c_price_ttm_pe_trend),
